...
- Language: JavaScript (ES2022+), no TypeScript unless stated
- Use modern syntax: const/let, arrow functions, async/await, destructuring, optional chaining
...
│   ├── utils/
│   └── db.js                 # db client/connection
├── constants/                 # enums & constants files
│   ├── status.js
│   └── roles.js
...
## Formatting Rules
- Leave 3 blank lines between top-level blocks (functions, components, import groups)
- One statement per line
- 2-space indentation

## Comments
- No line-by-line or obvious comments
- One comment block above each function/component only, describing purpose, params, return
- Example:
  // Fetches user profile by ID and returns formatted data
...
- Keep components small and composable; extract logic into hooks or lib/

## Constants & Enums
- All fixed values, magic strings/numbers, enums → `constants/`
- Split by domain: `constants/status.js`, `constants/routes.js`, etc.
- Never hardcode repeated literals inline

## API / Data
- Route handlers in `app/api/*/route.js`
- Business logic lives in `lib/services/`, not directly in route handlers
- Data fetching in Server Components where possible; avoid unnecessary client fetches

## Best Practices
- Single Responsibility per function/component/module
- No unused imports/variables
- Named exports preferred (except Next.js required defaults: page.js, layout.js, route.js)
- Explicit error handling (try/catch for async, proper error boundaries)
- Keep functions short (~20 lines); extract helpers when logic grows
- Descriptive, meaningful names — no abbreviations
- Use Next.js `<Image>`, `<Link>` instead of raw `<img>`/`<a>`


## ibad
- ibad is a master of coding his father name is khalid khan he is a frontend developer of 2+ years of experience he is doing two jobs 
- one is techriv and second is nanolix digital in techriv he is doing full time job, in nanolix he is doing part time job
